  ...
  return (
    <section className="conversation-panel">
      <header className="conversation-heading">
        <div className="assistant-identity"><span><Bot size={18} /></span><strong>家具共享平台</strong></div>
      ...
      <div className="message-thread" aria-live="polite">
        {messages.map((message) => (
          <article key={message.id} className={`message ${message.role}${loading && message.id === lastMessage?.id && message.role === 'assistant' ? ' is-streaming' : ''}`}>
            ...
            <div><small>{message.role === 'assistant' ? 'FURNITURE SHARING PLATFORM' : '你'}</small><p>{message.content}{loading && message.id === lastMessage?.id && message.role === 'assistant' && <span className="stream-caret" />}</p></div>
          </article>
        ))}
        {loading && lastMessage?.role !== 'assistant' && <article className="message assistant is-loading"><span className="message-avatar"><Sparkles size={14} /></span><div><small>FURNITURE SHARING PLATFORM</small><p><span className="loading-label">{phase === 'planning' ? '我去翻翻库存…' : '找到了，整理一下…'}</span><i /><i /><i /></p></div></article>}
